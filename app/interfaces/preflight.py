...
class Preflight:
    ...
    def check(payload: PreflightPayload) -> Dict[str, Any]:
        ...
        results = {}
        try:
            results["databaseSchemaCheck"] = Preflight.check_schemas_and_databases(payload)
    ...
    def check_schemas_and_databases(payload: PreflightPayload) -> Dict[str, Any]:
        ...
        connection = None
        try:
            schemas_results = Preflight.fetch_metadata(payload.credentials.get_credential_config())

            include_filter = json.loads(payload.form_data.include_filter)
            allowed_databases, allowed_schemas = Preflight.extract_allowed_schemas(schemas_results)
            check_success, missing_object_name = Preflight.validate_filters(include_filter, allowed_databases, allowed_schemas)

            return {
                "success": check_success,
                "successMessage": "Schemas and Databases check successful" if check_success else "",
                "failureMessage": f"Schemas and Databases check failed for {missing_object_name}" if not check_success else ""
            }
        except Exception as e:
    ...
    def tables_check(payload: PreflightPayload) -> Dict[str, Any]:
        ...
        connection = None
        try:
            normalized_include_regex, normalized_exclude_regex, exclude_table = Preflight.prepare_filters(payload.form_data)
    ...
    def prepare_filters(form_data) -> Tuple[str, str, str]:
        ...
        normalized_include_filter_list = Preflight.normalize_filters(include_filter, True)
        normalized_exclude_filter_list = Preflight.normalize_filters(exclude_filter, False)

        normalized_include_regex = "|".join(normalized_include_filter_list) if normalized_include_filter_list else ".*"
        normalized_exclude_regex = "|".join(normalized_exclude_filter_list) if normalized_exclude_filter_list else "$^"

        exclude_table = form_data.temp_table_regex if form_data.temp_table_regex else "$^"
